...
///
/// Entries can be bare idents (same-crate types) or paths. Use `crate::`
/// prefix for same-crate qualified paths; other paths are treated as
/// external crate references.
///
/// ```ignore
/// ffier::library_definition!("mylib",
///     Calculator, CalcError,                        // bare idents (same crate)
///     crate::submod::TextBuffer,                    // same crate, qualified
///     trait crate::traits::Processor,               // implementable trait
///     crate::traits::Fruit for crate::types::Apple, // trait impl
///     other_crate::Widget,                          // cross-crate
/// );
///
/// // In cdylib:
...
pub fn library_definition(input: TokenStream) -> TokenStream {
    ...
    // For each entry, compute two paths:
    // 1. alias_path: the __ffier_meta_* alias next to the type (for @reexport invocations)
    // 2. chain_path: the same alias but with $crate:: for same-crate entries (for the
    //    __ffier_*_library! macro that expands in the cdylib)
    ...
    for entry in &parsed.entries {
        let (alias_path, chain_path) = resolve_entry_paths(entry);
        reexport_invocations.push(quote! { #alias_path!(@reexport); });
...
/// Compute the alias path and chain path for a library entry.
///
/// - alias_path: used at `library_definition!` expansion site (library crate root)
///   for `@reexport` invocations. Uses the path as-is.
/// - chain_path: used inside the generated `#[macro_export]` library macro that
///   expands in the cdylib. Same-crate paths get `$crate::` prefix; external
///   crate paths are used as-is.
fn resolve_entry_paths(
    entry: &LibraryEntry,
) -> (proc_macro2::TokenStream, proc_macro2::TokenStream) {
    match entry {
        LibraryEntry::Plain(path) => {
            let alias = meta_alias_for_type(path);
            let chain = to_chain_path(&alias);
            (quote! { #alias }, chain)
        }
        LibraryEntry::Trait(path) => {
            let alias = meta_alias_for_type(path);
            let chain = to_chain_path(&alias);
            (quote! { #alias }, chain)
        }
...
enum LibraryEntry {
    /// A plain type or error: `Calculator` or `crate::submod::Calculator`
    Plain(syn::Path),
    /// An implementable trait: `trait Processor` or `trait crate::traits::Processor`
    Trait(syn::Path),
    /// A trait impl bridge: `Fruit for Apple` or `crate::Fruit for crate::Apple`
    TraitImpl {
        trait_path: syn::Path,
    ...
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        while !input.is_empty() {
            if input.peek(Token![trait]) {
                // `trait Path::To::TraitName`
                input.parse::<Token![trait]>()?;
                let path: syn::Path = input.parse()?;
                entries.push(LibraryEntry::Trait(path));
            } else {
